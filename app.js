(() => {
  ...
  function markEquity() {
    ...
    if (state.status === "active") {
      if (dailyDd >= state.dailyDdPct / 100 || maxDd >= state.maxDdPct / 100) {
        ...
        state.status = "passed"; state.autopilot = false; log("Target hit");
      }
    }
    const now = Date.now();
  ...
  $("btnArm").onclick = () => { state.bots.forEach((b) => (b.enabled = true)); state.autopilot = true; adaptRegime(); log("Max edge armed"); scheduleRender(); };
  ...
  $("btnReset").onclick = () => {
    ...
    log("Reset $50k — turbo live"); scheduleRender();
  };
  ...
  setInterval(() => { if (state.autopilot && state.status === "active") document.title = `Northline ${fmtPct(((state.equity - state.size) / state.size) * 100)} · Live Edge`; }, 1200);
  scheduleRender();
})();

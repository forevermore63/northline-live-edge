(() => {
  ...
  function markEquity() {
    ...
    if (state.status === "active") {
      if (dailyDd >= state.dailyDdPct / 100 || maxDd >= state.maxDdPct / 100) {
        ...
        state.status = "passed"; state.autopilot = false; log("Target hit — Pivex $100k edge");
      }
    }
    const now = Date.now();
  ...
  function connectWs() {
    ...
    ws.onopen = () => {
      ...
      log("WS live — Pivex $100k turbo armed");
      scheduleRender();
    };
    ws.onmessage = (ev) => {
      try {
  ...
  $("btnArm").onclick = () => { state.bots.forEach((b) => (b.enabled = true)); state.autopilot = true; adaptRegime(); log("Max edge armed · $100k"); scheduleRender(); };
  ...
  $("btnReset").onclick = () => {
    ...
    log("Reset $100k — turbo live · Pivex scale"); scheduleRender();
  };
  ...
  log("v2.1 turbo boot — Pivex $100k challenge");
  ...
  setInterval(() => { if (state.autopilot && state.status === "active") document.title = `Northline ${fmtPct(((state.equity - state.size) / state.size) * 100)} · $100k`; }, 1200);
  scheduleRender();
})();
